...
class GitSimBaseCommand:
    ...
    def parse_commits(
        self, commit, prevCircle=None, shift=numpy.array([0.0, 0.0, 0.0]),
    ):
        if self.stop:
            return
    ...
    def show_intro(self):
        if self.scene.args.animate and self.scene.args.show_intro:
            ...
            self.scene.play(
                self.logo.animate.scale(0.25).to_edge(UP, buff=0).to_edge(RIGHT, buff=0),
            )
    ...
    def draw_commit(self, commit, prevCircle, shift=numpy.array([0.0, 0.0, 0.0])):
        ...
        if prevCircle:
            circle.next_to(
                prevCircle, RIGHT if self.scene.args.reverse else LEFT, buff=1.5,
            )

        start = (
            prevCircle.get_center()
            if prevCircle
            else (LEFT if self.scene.args.reverse else RIGHT)
        )
        end = circle.get_center()

        if commit == "dark":
            arrow = Arrow(
                start, end, color=WHITE if self.scene.args.light_mode else BLACK,
            )
    ...
    def draw_branch(self, commit):
        ...
        for branch in branches:
            ...
            if commit.hexsha == self.repo.heads[branch].commit.hexsha:
                ...
                if self.scene.args.animate:
                    self.scene.play(
                        Create(fullbranch), run_time=1 / self.scene.args.speed,
                    )
                else:
    ...
    def vsplit_frame(self):
        ...
        try:
            if self.scene.args.animate:
                self.scene.play(
                    self.toFadeOut.animate.align_to(self.scene.camera.frame, UP).shift(
                        DOWN * 0.75,
                    ),
                )
            else:
    ...
    def setup_and_draw_zones(
        ...
        firstColumnTitle = (
            ...
            .align_to(self.scene.camera.frame, LEFT)
            .shift(RIGHT * 0.65)
            .shift(UP * self.zone_title_offset)
        )
        secondColumnTitle = (
            Text(
                second_column_name,
        ...
        thirdColumnTitle = (
            ...
            .align_to(self.scene.camera.frame, RIGHT)
            .shift(LEFT * 1.65)
            .align_to(firstColumnTitle, UP)
        )
        ...
        if self.scene.args.animate:
            self.scene.play(
                ...
                Create(vert1),
                Create(vert2),
                AddTextLetterByLetter(firstColumnTitle),
                AddTextLetterByLetter(secondColumnTitle),
                AddTextLetterByLetter(thirdColumnTitle),
            )
        else:
            self.scene.add(
                horizontal,
        ...
        for i, f in enumerate(firstColumnFileNames):
            text = (
                Text(f, font="Monospace", font_size=24, color=self.scene.fontColor)
                .move_to(
    ...
    def draw_dark_ref(self):
        refRec = Rectangle(
            color=WHITE if self.scene.args.light_mode else BLACK,
            fill_color=WHITE if self.scene.args.light_mode else BLACK,
            height=0.4,
            width=1,
        )
